...
resource "aws_vpc" "vpc" {
  cidr_block = var.vpc_cidr # ie for dev 10.1.0.0/16
...
resource "aws_subnet" "public_subnets" {
  ...
  tags = {
    ...
    # dev-public-az1, the next dev-public-az2
    Environment = "development"
  }
}
...
resource "aws_launch_template" "web_lt" {
  ...
  user_data = base64encode(<<-EOF
              ...
              echo "<h1>dev ENVIRONMENT | Host: $(hostname -f)</h1>" > /var/www/html/index.html
              EOF
  )
}
...
resource "aws_lb_target_group" "web_tg" {
  ...
  health_check {
    ...
    protocol            = "HTTP"
    matcher             = "200"
    interval            = 15 # Faster checks for dev
    timeout             = 3
    healthy_threshold   = 2
